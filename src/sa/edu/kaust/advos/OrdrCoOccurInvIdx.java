...
public class OrdrCoOccurInvIdx {
	...
	extends Reducer<IntTuple,IntWritable,Text,Text> {
		...
		public void reduce(IntTuple key, Iterable<IntWritable> values, 
			...
			if (key.getKey3() == 0) { // previous master_list finished
				emitHashMap(context);
			}
			// put the new list in storage
			map.put(key.clone(), new IntWritable(sum));
		}
	}
	...
	public static void main(String[] args) throws Exception {
		...
		if (otherArgs.length != 3) {
			System.err.println("Usage: OrdrCoOccurInvIdx <w> <in> <out>");
			System.exit(2);
		}
	    conf.set(w_env_name, otherArgs[0]);
	    System.err.println("Main* Conf.get : " + conf.get(w_env_name));
	    
		Job job = new Job(conf, "Ordered Co-Occurence Inverted Index");
